...
from database import init_db, get_all_pages, insert_page
...
if "oyobidashi_flags" not in st.session_state:
    st.session_state["oyobidashi_flags"] = {}  # { post_index: True/False }
        ...
        for rank, post in enumerate(post_top10, start=1):
            ...
            with st.container(border=True):
                ...
                with col_body:
                    ...
                    preview = post["commeent"][:40] + "…" if len(post["commeent"]) > 40 else post["commeent"]
                    with st.expander(preview):
                        st.caption(f"{name}")
                        st.write(post["commeent"])

                with col_like:
                    st.metric("❤️", post["good"])
